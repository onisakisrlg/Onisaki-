...
export const Collection: React.FC = () => {
  return (
    <section id={Section.SERVICES} className="py-24 bg-oni-light-bg/90 dark:bg-oni-bg/90 backdrop-blur-sm border-t border-black/5 dark:border-white/5 transition-colors duration-500">
      <div className="container mx-auto px-6">
        <div className="text-center mb-20 space-y-4">
          ...
          <Reveal delay={0.2} direction="down">
             ...
             <p className="text-gray-500 dark:text-gray-400 font-sans tracking-widest text-xs uppercase font-bold">Comprehensive Digital Solutions</p>
          </Reveal>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {SERVICES.map((service, index) => (
            <Reveal key={service.id} delay={index * 0.1}>
              <div className="group relative bg-white/80 dark:bg-oni-card/80 p-10 h-full border border-black/5 dark:border-white/5 transition-all duration-500 backdrop-blur-md flex flex-col justify-between overflow-hidden rounded-xl shadow-lg dark:shadow-none hover:-translate-y-2">
                
                {/* 
                   =======================
                   HOVER-ONLY CONTINUOUS LOOP
                   =======================
                   Added 'group-hover:' prefix so animation only plays when interacting.
                   The base state uses translate/offset to keep lines hidden until hover.
                */}
                ...
                {/* Inset by 6px, rounded to match concentric curvature */}
                ...
                <div>
                  ...
                  <div className="relative z-10 mb-8 inline-block">
                    <div className="w-24 h-24 p-2 transition-transform duration-500 group-hover:scale-110 group-hover:-translate-y-2">
                       {/* Subtle floor glow */}
                       <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-16 h-4 bg-oni-cyan/20 blur-lg rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                       {service.icon}
                    </div>
                  </div>
                  ...
                  <div className="relative z-10">
                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2 group-hover:text-oni-cyan transition-colors">{service.title}</h3>
                    <h4 className="text-xs text-oni-purple uppercase tracking-widest mb-6 font-bold">{service.subtitle}</h4>
                    <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed mb-8">{service.description}</p>
                  </div>
                </div>
                 
                <div className="relative z-10 flex items-center gap-2 text-gray-800 dark:text-white text-xs font-bold tracking-widest uppercase group-hover:text-oni-magenta transition-colors mt-auto">
